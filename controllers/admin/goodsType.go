...
func (con GoodsTypeController) DoEdit(c *gin.Context) {
	...
	goodsType := models.GoodsType{Id: id} //根据orm传入的id,去修改数据
	models.DB.Find(&goodsType)            //找到这个id对应的东西
	goodsType.Title = title               //以下开始修改数据
...
func (con GoodsTypeController) Delete(c *gin.Context) {
	id, err := models.Int(c.Query("id")) //查询query表单
	if err != nil {
		con.Error(c, "传入数据错误", "/admin/goodsType")
	} else {
		goodsType := models.GoodsType{Id: id} //根据id使用orm查询
