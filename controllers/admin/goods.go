...
func (con GoodsController) DoAdd(c *gin.Context) {
	...
	go func() { //开启协程增加一个数据
		...
		for _, v := range goodsImageList { //因为获取到的是一个slice
			goodsImgObj := models.GoodsImage{} //实例化orm对象
			goodsImgObj.GoodsId = goods.Id     //赋值
			goodsImgObj.ImgUrl = v
			goodsImgObj.Sort = 10
			goodsImgObj.Status = 1
			goodsImgObj.AddTime = int(models.GetUnix())
			models.DB.Create(&goodsImgObj) //执行orm操作，
		}
		wg.Done() //Done
	}()
	//6、增加规格包装
